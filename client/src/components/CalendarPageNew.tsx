...
export function CalendarPage() {
  ...
  const generateRandomBooking = () => {
    ...
    setFormData({
      ...
      clientEmail: `${names[Math.floor(Math.random() * names.length)].toLowerCase().replace(' ', '.')}@email.cz`,
  ...
  const filteredBookings = BookingService.filterBookingsByStatus(
    selectedDate ? BookingService.getBookingsForDate(bookings, selectedDate) : bookings,
    statusFilter
  );
              ...
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-xl font-semibold text-gray-800">
                    Rezervace ({filteredBookings.length})
                  </h2>
                </div>
              ...
              {loading ? (
                ...
                <div className="space-y-4">
                  {filteredBookings.map((booking) => (
                    <div
                      key={booking.id}
                      className="bg-gradient-to-r from-violet-50 to-purple-50 rounded-2xl p-6 border border-violet-200/50 hover:shadow-lg transition-all duration-200 hover:scale-[1.02]"
                    >
